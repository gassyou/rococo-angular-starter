...
export function encryptForServer(text: string): string {
    ...
    try {
        ...
        const password_binary = btoa(CryptoJS.AES.encrypt(password_text, secret_passphrase,
...
export function decryptForServer(text: string): string {
    try {
        if (text !== null && text !== '') {
            ...
            const options = {iv: iv, mode: CryptoJS.mode.CBC, padding: CryptoJS.pad.Pkcs7};
            const decrypted = CryptoJS.AES.decrypt(encrypted_data,
                secret_passphrase, options);
